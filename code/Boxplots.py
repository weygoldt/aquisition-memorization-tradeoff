...
new_colors = cmocean.tools.crop_by_percent(cmocean.cm.ice, 40, which='both')
...
sns.boxplot(x='cpds', y='err_rate', hue='delay', data=error_rate, ax=ax[0], palette = ndc)
...
sns.boxplot(x='cpds', y='mean', hue='delay', data=resp_time, ax=ax[1], palette = ndc)
...
fig, ax = plt.subplots(1,2, figsize=(8,4))
fig.tight_layout(pad=2.5)
sns.boxplot(x='cpds', y='mean', hue='delay', data=shifts, ax=ax[0], palette = ndc)
ax[0].get_legend().remove()
ax[0].set_ylabel('number of gaze shifts')
ax[0].spines['top'].set_visible(False)
ax[0].spines['right'].set_visible(False)
ax[0].spines['bottom'].set_linewidth(2.0)
ax[0].spines['left'].set_linewidth(2.0)
...
sns.boxplot(x='cpds', y='mean', hue='delay', data=proc_time, ax=ax[1], palette = ndc)
