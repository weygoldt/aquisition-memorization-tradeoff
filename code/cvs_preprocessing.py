...
def dataimport(dataroot: str, processed_dataroot: str) -> pd.DataFrame:
    ...
    for i, file in enumerate(os.listdir(dataroot)):
        ...
        df.columns = ['trialno', 'correctno', 'delay',
                      'cpd', 'selected', 'totdur', 'switches', 'tpertrial']
...
for name, color in zip(df_all.names, colors):
    ...
    plt.errorbar(minidf.switches_means, minidf.tpertrial_means,
                 xerr=minidf.switches_sems, yerr=minidf.tpertrial_sems, fmt='bo', label=name, color=color)
plt.legend()
plt.show()
