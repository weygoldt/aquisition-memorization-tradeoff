...
if __name__ == "__main__":
    ...
    data = getindex(df[df.delay == 1.5])
    newdata = data.sort_values(by=['mean'], ascending=True).reset_index()
    for i, name in enumerate(newdata.name):
        plt.errorbar(newdata['mean'][i], i, xerr=newdata['sem'][i], fmt='o', c=colors1[i], capsize=3, label='2 s delay'if i == 6 else "")
    
    data = getindex(df[df.delay == 0])
    newdata = data.sort_values(by=['mean'], ascending=True).reset_index()
    for i, name in enumerate(newdata.name):
        plt.errorbar(newdata['mean'][i], i+0.3, xerr=newdata['sem'][i], fmt='o', c=colors2[i], capsize=3, label='0 s delay' if i == 6 else "")
