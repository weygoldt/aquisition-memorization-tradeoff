...
popt0, pcov = curve_fit(
    func_powerlaw, ms_0.switches_means, ms_0.tpertrial_means, method="trf", maxfev=50000
)
popt2, pcov = curve_fit(
    func_powerlaw, ms_2.switches_means, ms_2.tpertrial_means, method="trf", maxfev=50000
)
...
ax.plot(
    x, func_powerlaw(x, *popt0), color=colors[1], label="2 cpd fit", lw=2, zorder=10
)
ax.plot(
    x, func_powerlaw(x, *popt2), color=colors[3], label="8 cpd fit", lw=2, zorder=10
)
...
x = np.linspace(ms_2.switches_means.min() - 0.1, ms_2.switches_means.max() + 2, 1000)
axins.plot(x, func_powerlaw(x, *popt0), color=colors[1], lw=2, zorder=10)
axins.plot(x, func_powerlaw(x, *popt2), color=colors[3], lw=2, zorder=10)
